...
const hostname = window.location.hostname === 'localhost' ? 'localhost:8000' : 'pmc-qms.com';
...
export default function TvViewer() {
  ...
  const speakNext = () => {
    ...
    const ticket = speechQueueRef.current.shift();
    
    const bellAudio = new Audio('/bell.mp3');
    
    bellAudio.onended = () => {
      // Option A: Native Web Speech API
      const msg = new SpeechSynthesisUtterance(`Ticket number ${ticket.ticket_number}, please proceed to ${ticket.division_name}.`);
      
      // Select best voice if available (optional)
  ...
  const speak = (ticket) => {
    if (ticket && ticket.ticket_number && ticket.division_name) {
      speechQueueRef.current.push(ticket);
      speakNext();
    }
  };
